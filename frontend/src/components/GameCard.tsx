import { Card, Center, Wrap, WrapItem } from "@chakra-ui/react";
...
//no key, diff callback (no arguments)
...
const GameCard = ({ index, callback, text, id, type }: GameCardProps) => {
  return (
      <WrapItem>
        <Card
          border={
            type === "newTemplate" ? `2px gray dashed` : `2px black solid`
          }
          bgGradient={
            type === "newTemplate"
          ...
          padding={"10px"}
          onClick={
            type === "newTemplate" ? () => callback() : () => callback(id)
          }
          key={index}
        >
